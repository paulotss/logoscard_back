...
class PagBankService {
  ...
  public static async getCustomers(): Promise<PagBankCustomersResponse> {
    ...
    try {
      const response = await axios.get<PagBankCustomersResponse>(url, {
        headers,
      });
      return response.data;
    } catch (error) {
  ...
  public static async findByCPF(cpf: string): Promise<{ id: string } | null> {
    try {
      ...
      const { customers } = pagbankCustomerData;
  ...
  public static async createSignature(
    data: any,
  ): Promise<{ data: PagBankSubscriptionResponse }> {
    const baseUrl = process.env.PAGBANK_API_SUBSCRIPTIONS_URL;
    const url = `${baseUrl}/subscriptions`;
    const headers = this.getHeaders();
    console.log('GGG my dataaa', JSON.stringify(data, null, 2));
    const payload = {
      plan: data.plan,
    ...
    try {
      const response = await axios.post<PagBankSubscriptionResponse>(
        url,
        payload,
        { headers },
      );
      console.log('userId', userId);
      await UserService.updateSubscriptionData(
        userId,
        data.reference_id,
        response.data.id,
      );

      // Generate invoices based on PagBank subscription response
      try {
        const billingDay = new Date(response.data.next_invoice_at).getDate();
        const monthlyAmount = response.data.amount.value / 100; // Convert from cents to currency unit
        const paymentMethodType =
          response.data.payment_method[0]?.type || 'CREDIT_CARD';
        const paymentMethod =
          paymentMethodType === 'CREDIT_CARD' ? 'CRÉDITO' : paymentMethodType;

        // Get user dependents count
        const dependents = await UserService.getDependentsCount(userId);

        // Extract billing information from plan
        const planInterval = response.data.plan.interval;
        // Use the plan's interval length as the number of billing cycles
        const parcels = planInterval.length;
        ...
        console.log(
          `Generated ${parcels} invoices for subscription ${response.data.id}`,
        );
      } catch (invoiceError) {
        console.error('Error generating invoices:', invoiceError);
        // Note: We don't throw here to avoid breaking the subscription creation
        // The subscription was created successfully, invoice generation is secondary
      }
  ...
  public static async getSubscription(
    ...
    try {
      const response = await axios.get<PagBankSubscriptionResponse>(url, {
        headers,
      });
      return response.data;
    } catch (error) {
